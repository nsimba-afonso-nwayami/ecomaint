...
export default function OcorrenciasAdmin() {
  ...
  const [openExcluirOcorrencia, setOpenExcluirOcorrencia] = useState(false);
  ...
  // Dados mockados focados em gestão de falhas e infraestrutura industrial
  const ocorrencias = [
    {
      id: "OCR-001",
  ...
  const handleExcluirOcorrencia = (ocr) => {
    setOcorrenciaSelecionada(ocr);
    setOpenExcluirOcorrencia(true);
  };
  ...
  return (
    <>
      ...
      <AdminLayout title="Ocorrências">
        <section className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-white p-4 rounded-xl border border-slate-100 shadow-xs">
            ...
            <div className="flex items-center justify-end pr-2 text-xs font-bold text-slate-400">
              Total: <span className="text-green-900 ml-1 font-black">{ocorrencias.length} Ocorrências</span>
            </div>
          </div>
            ...
            {ocorrencias.map((ocr) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:flex sm:flex-row sm:items-center gap-4 sm:gap-8 text-xs sm:px-4">
                  ...
                  {/* Data do Evento */}
                  <div className="flex flex-col gap-0.5">
                    <span className="text-slate-400 font-medium sm:text-[10px] sm:uppercase sm:tracking-wider">Data de Abertura</span>
                    <span className="font-semibold text-slate-600 flex items-center gap-1">
                      <i className="fa-regular fa-calendar text-[11px] text-slate-400" /> {ocr.data}
                    </span>
                  </div>
                ...
                <div className="border-t lg:border-t-0 border-slate-50 pt-3 lg:pt-0 flex items-center justify-between lg:justify-end shrink-0 lg:pl-4">
                  ...
                  <div className="flex gap-1">
                    ...
                    <button 
                      onClick={() => handleExcluirOcorrencia(ocr)}
        ...
        <ModalSmall
          isOpen={openExcluirOcorrencia}
          onClose={() => setOpenExcluirOcorrencia(false)}
          title="Arquivar Ocorrência"
          icon="fa-solid fa-box-archive"
        >
          <div className="p-1 space-y-4 text-center sm:text-left">
            <p className="text-slate-600 text-sm leading-relaxed">
              Tem a certeza de que pretende remover permanentemente a ocorrência ativa no equipamento{" "}
              ...
              ? Esta ação irá retirá-lo da lista de incidentes críticos imediatos da fábrica.
            </p>

            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <button
                onClick={() => setOpenExcluirOcorrencia(false)}
